...
inline int select_read(socket_t sock, time_t sec, time_t usec)
{
    ...
    timeval tv;
    tv.tv_sec = (decltype(tv.tv_sec))sec;
    tv.tv_usec = (decltype(tv.tv_usec))usec;
...
{
    return recv(sock_, ptr, (int)size, 0);
}

inline int SocketStream::write(const char* ptr, size_t size)
{
    return send(sock_, ptr, (int)size, 0);
}
...
inline socket_t Server::create_server_socket(const char* host, int port, int socket_flags) const
{
    return detail::create_socket(host, port,
        [](socket_t sock, struct addrinfo& ai) -> bool {
            if (::bind(sock, ai.ai_addr, (int)ai.ai_addrlen)) {
                  return false;
            }
...
inline socket_t Client::create_client_socket() const
{
    return detail::create_socket(host_.c_str(), port_,
        [=](socket_t sock, struct addrinfo& ai) -> bool {
            ...
            auto ret = connect(sock, ai.ai_addr, (int)ai.ai_addrlen);
            if (ret < 0) {
